...
class TestCompareCommand:
    ...
    def test_compare_with_instance_type(self, mock_check_od, mock_pricing, mock_ec2):
        ...
        mock_ec2_client.describe_spot_price_history.return_value = {
            'SpotPriceHistory': [
                {'SpotPrice': '1.50', 'Timestamp': datetime.now(timezone.utc)}
            ]
        }
...
class TestCompareEdgeCases:
    ...
    def test_compare_with_multiple_instance_types(self, mock_check_od, mock_pricing, mock_ec2):
        ...
        result = main(
            job_spec='{"t3.micro": 2, "t3.small": 3}',
            output='json',
            regions=['us-east-1']
        )
...
class TestCompareDiscovery:
    ...
    @patch('aws_fleet_scout.commands.compare.get_pricing_client')
    @patch('aws_fleet_scout.commands.compare.check_on_demand_availability')
    def test_compare_with_discover_p_series(self, mock_check_od, mock_pricing, mock_ec2, mock_discover):
    ...
    @patch('aws_fleet_scout.commands.compare.get_pricing_client')
    @patch('aws_fleet_scout.commands.compare.check_on_demand_availability')
    def test_compare_with_discover_custom_prefix(self, mock_check_od, mock_pricing, mock_ec2, mock_discover):
    ...
    @patch('aws_fleet_scout.commands.compare.get_pricing_client')
    @patch('aws_fleet_scout.commands.compare.check_on_demand_availability')
    def test_compare_with_discover_multiple_prefixes(self, mock_check_od, mock_pricing, mock_ec2, mock_discover):
    ...
    def test_compare_with_discover_no_instances_found(self, mock_discover):
        ...
        result = main(
            discover='p99',  # Non-existent prefix
            output='json',
            regions=['us-east-1']
        )
