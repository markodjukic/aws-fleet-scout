...
from .config import PRICING_REGION_MAP, DEFAULT_REGIONS
...
def prompt_discovery() -> Optional[str]:
    ...
    prefix = questionary.text(
        "Instance prefix(es) to discover (comma-separated):",
        default="p5",
        style=custom_style
    ).ask()
...
def build_compare_command() -> Optional[List[str]]:
    ...
    # Output format
    output_table = questionary.confirm(
        "Table output?",
        default=True,
        style=custom_style
    ).ask()
...
def build_capacity_find_command() -> Optional[List[str]]:
    ...
    duration = questionary.text(
        "Duration in days (1-14 daily, or 21-182 in 7-day steps):",
        default="1",
        style=custom_style
    ).ask()
    ...
    max_days = questionary.text(
        "Search window (days ahead, typically max ~10 days):",
        default="7",
        style=custom_style
    ).ask()
...
def build_fleet_pack_command() -> Optional[List[str]]:
    ...
    job_spec = questionary.text(
        'Job spec (JSON):',
        default='{"m7i.4xlarge": 20, "p5.48xlarge": 2}',
        style=custom_style
    ).ask()
