...
def get_available_regions_for_instance(instance_type: str, regions: Optional[List[str]] = None) -> List[str]:
    """
    ...
    for region in regions:
        ...
        def fetch_offerings():
            try:
                ...
                return [offering['InstanceType'] for offering in response.get('InstanceTypeOfferings', [])]
            except Exception:
                return []
            ...
            def fetch_offerings():
                try:
                    ...
                    for page in paginator.paginate(
                        LocationType='region',
                        Filters=[{'Name': 'location', 'Values': [r]}]
                    ):
                        all_types.extend(
        ...
        instance_types = get_cached_data('instance_offerings', make_fetch_offerings(region), region=region)
