...
def main(
    ...
    if discover or discover_p_series:
        ...
        if len(regions) != 1:
            if ctx:
                typer.echo(ctx.get_help())
                typer.echo(
                    f"\nError: Discovery mode requires exactly one region. Got {len(regions)}: {regions}",
                    err=True,
                )
                typer.echo("Hint: Use --regions us-east-1 (single region only)", err=True)
                raise typer.Exit(1)
            raise typer.BadParameter(
                f"Discovery mode requires exactly one region. Got {len(regions)}"
            )
    ...
    if output == "json":
        ...
        if discovery_mode:
            ...
            _print_discovery_calendar_view(
                calendar_data, instance_requirements, duration, duration_hours, window, regions[0]
            )
        else:
            # Normal mode: regions as rows, dates as columns
            _print_calendar_view(
                calendar_data, instance_requirements, duration, duration_hours, window, regions
            )
...
def _print_calendar_view(  # pragma: no cover
        ...
        if total_offerings > 0:
            ...
            if all_offerings:
                best_date, best_region, best_offering = min(
                    all_offerings, key=lambda x: (x[2]["start_date"], x[2]["upfront_fee"])
                )
                ...
                console.print(
                    f"  Start: {best_offering['start_date'].strftime('%Y-%m-%d %H:%M UTC')}"
                )
    ...
    # Legend
    console.print(
        "[dim]Legend: Price shown for cheapest offering, - = Not available, * = Today[/dim]"
    )
