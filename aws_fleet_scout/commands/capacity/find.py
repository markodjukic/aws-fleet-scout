    ...
    if discover or discover_p_series:
        ...
        if output != 'json':
            ...
            if len(discovered_instances) > 10:
                ...
                try:
                    response = input(f"Proceed with querying {len(discovered_instances)} instance types? [y/N]: ")
        ...
        try:
            ...
            if ctx:
                typer.echo(ctx.get_help())
                typer.echo(f"\nError: Invalid job_spec JSON: {e}. Expected format: '{{\"instance_type\": quantity, ...}}'", err=True)
                raise typer.Exit(1)
            raise typer.BadParameter(f"Invalid job_spec JSON: {e}. Expected format: '{{\"instance_type\": quantity, ...}}'")
    else:
        if ctx:
            typer.echo(ctx.get_help())
...
def _find_best_offerings(
    all_offerings: Dict[str, Dict[str, List[Dict]]],
    ...
    for instance_type, offerings_by_region in all_offerings.items():
        ...
        if all_region_offerings:
            ...
            best = min(all_region_offerings, key=lambda x: (
                x['start_date'],
                x['duration_hours'],
                x['upfront_fee']
            ))
    ...
    results.sort(key=lambda x: (x['start_date'] is None, x['start_date'] or datetime.max.replace(tzinfo=timezone.utc)))
    
    return results


def _print_table(offerings: List[Dict], requirements: Dict[str, int], duration_days: int, duration_hours: int, max_days: int):  # pragma: no cover
    ...
    if not any(o['start_date'] for o in offerings):
        ...
        console.print("   Try expanding your search to additional regions or adjusting the time window.\n")
        return
    
    table = Table(title=f"Capacity Block Offerings (Duration: {duration_days}d/{duration_hours}hrs, Window: {max_days} days)", show_header=True, header_style="bold magenta")
    ...
    for offering in offerings:
        if offering['start_date']:
            ...
            offering_id = offering['offering_id'][:18] if len(offering['offering_id']) > 18 else offering['offering_id']
