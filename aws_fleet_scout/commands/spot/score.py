...
from ...utils.cache import get_cached_data, CACHE_DIR
...
def main(
    ...
    if output != 'json' and len(instance_types) > 1 and cache_hits > 0:
        print()
        print(f"ℹ️  {cache_hits}/{len(instance_types)} results from cache (10 min TTL, use --force-refresh to bypass)")
