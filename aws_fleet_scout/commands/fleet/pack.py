...
def main(job_spec: str = "", output: str = "json", regions: List[str] = None, ctx: typer.Context = None):
    """
    ...
    if not job_spec:
        if ctx:
            ...
            typer.echo("\nError: job_spec is required. Example: '{\"m7i.4xlarge\": 20, \"p5.48xlarge\": 2}'", err=True)
            raise typer.Exit(1)
        raise typer.BadParameter("job_spec is required. Example: '{\"m7i.4xlarge\": 20, \"p5.48xlarge\": 2}'")
    ...
    sorted_regions = sorted(region_scores.items(), key=lambda x: x[1]['aggregate_score'], reverse=True)
    ...
    if output == 'json':
        ...
        for region, data in sorted_regions:
            scores_str = ', '.join([f"{itype}: {score}" for itype, score in data['instance_scores'].items()])
            table.add_row(
                region,
                data['az_id'],
                f"{data['aggregate_score']:.2f}",
                scores_str
            )
...
def _compute_region_scores(instance_scores: Dict[str, List[Dict]], job_requirements: Dict[str, int]) -> Dict[str, Dict]:
    """
    ...
    for (region, az_id), scores_dict in region_data.items():
        ...
        weighted_sum = sum(scores_dict[itype] * job_requirements[itype] for itype in job_requirements)
