...
def recommend_placement(
    ...
    viable_options = _apply_constraints(
    ...
    if not viable_options:
        if output != 'json':
            typer.echo(f"Error: No viable options found with constraints (min_score >= {constraints.min_score_threshold})", err=True)
        raise typer.Exit(1)
    ...
    if output == 'json':
        ...
        for idx, option in enumerate(viable_options, 1):
            scores_str = ', '.join([f"{itype}: {score}" for itype, score in option.instance_scores.items()])
            table.add_row(
                str(idx),
                option.region,
...
def main(
    ...
    constraints = PlacementConstraints(
        ...
        required_instance_families=[f.strip() for f in required_families.split(",") if f.strip()] if required_families else None,
        region_allowlist=[r.strip() for r in region_allowlist.split(",") if r.strip()] if region_allowlist else None,
        region_denylist=[r.strip() for r in region_denylist.split(",") if r.strip()] if region_denylist else None,
        network_adjacency=network_adjacency if network_adjacency else None,
        gpu_families=[g.strip() for g in gpu_families.split(",") if g.strip()] if gpu_families else None
    )
