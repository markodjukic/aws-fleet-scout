...
def _get_capacity_block_data(
    instance_type: str, quantity: int, duration: int, max_days: int, regions: List[str]
) -> Dict:
    ...
    try:
        ...
        for region in available_regions:
            ...
            for price_item_str in response.get("PriceList", []):
                ...
                if (
                    attrs.get("preInstalledSw", "").lower() in ["na", "n/a", ""]
                    and attrs.get("capacitystatus", "").lower() == "used"
                ):
...
def _print_comparison_table(
    results: Dict, requirements: Dict, regions: List[str]
):  # pragma: no cover
    ...
    for instance_type, data in results.items():
        ...
        table = Table(
            title=f"{instance_type} (Quantity: {qty})",
            show_header=True,
        ...
        if spot["available"]:
            cost_str = (
                f"${spot['price_per_hour']:.4f}/hr (${spot['total_hourly_cost']:.2f}/hr total)"
            )
        ...
        if cb["available"]:
            ...
            start_str = (
                "Immediate" if time_until <= 1 else cb["start_date"].strftime("%Y-%m-%d %H:%M UTC")
            )
        ...
        if od["available"] and od["price_per_hour"]:
            cost_str = (
                f"${od['price_per_hour']:.4f}/hr (${od['price_per_hour'] * qty:.2f}/hr total)"
            )
            details = od["note"]
            available = "✓"
        elif od["available"]:
