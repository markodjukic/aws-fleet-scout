...
class CapacityOptimizer:
    ...
    def find_best_region_for_fleet(self, requirements):
        ...
        result = fleet_packing(job_spec=job_spec, output="json", regions=self.default_regions)
...
def example_simple_region_selection():
    ...
    requirements = {"m7i.4xlarge": 20, "p5.48xlarge": 2}
...
def example_validate_before_deployment():
    ...
    requirements = {"c6i.32xlarge": 50, "p4d.24xlarge": 10}

    is_feasible, result = optimizer.validate_deployment_feasibility(
        requirements, min_score=7.0  # Require high confidence
    )
...
def example_multi_region_strategy():
    ...
    requirements = {"m6i.16xlarge": 10, "r6i.8xlarge": 5}
    ...
    job_spec = json.dumps(requirements)
    full_result = fleet_packing(job_spec=job_spec, output="json", regions=regions_to_check)
...
def example_cicd_integration():
    ...
    optimizer = CapacityOptimizer()
    is_feasible, result = optimizer.validate_deployment_feasibility(
        DEPLOYMENT_CONFIG[environment], min_score=6.0
    )
